...
const Header = () => {
  ...
  const closeMobileNav = () => setNavigationOpen(false);

  const cartButton = (
    <button
      type="button"
      onClick={handleOpenCartModal}
      className="flex items-center gap-2 rounded-brutal border-2 border-ink bg-white px-2.5 py-1.5 shadow-brutal-sm transition-all duration-150 hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none active:translate-x-0.5 active:translate-y-0.5 active:shadow-none sm:gap-2.5 sm:px-3 sm:py-2"
    >
      <span className="relative inline-block">
        ...
        <span className="absolute -right-2.5 -top-2.5 flex h-5 w-5 items-center justify-center rounded-full border-2 border-ink bg-sunny-yellow text-2xs font-bold text-ink">
          {product.length}
        </span>
      </span>
      <div className="hidden text-left sm:block">
        <span className="block text-2xs font-semibold uppercase tracking-wide text-dark-4">
          cart
        </span>
        <p className="font-bold text-custom-sm text-ink">
  ...
  const menuToggleButton = (className = "") => (
    <button
      id="Toggle"
      type="button"
      ...
      className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-brutal border-2 border-ink bg-sunny-yellow text-ink shadow-brutal-sm transition-all duration-150 hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none active:shadow-none ${navigationOpen ? "bg-green-600 text-white" : ""} ${className}`}
    >
      {navigationOpen ? (
  ...
  const navigationMenu = (onNavigate?: () => void, variant: "mobile" | "desktop" = "desktop") => (
    <nav className="w-full">
      <ul
        className={`flex flex-col gap-2 xl:flex-row xl:items-center xl:gap-1 ${
          variant === "mobile" ? "gap-2" : ""
        }`}
      >
        {menuData.map((menuItem, i) =>
          menuItem.submenu ? (
            <Dropdown key={i} menuItem={menuItem} stickyMenu={stickyMenu} variant={variant} />
          ) : (
            <li key={i} className="group relative xl:before:hidden">
              <Link
                href={menuItem.path}
                onClick={onNavigate}
                className={`flex rounded-brutal border-2 border-transparent px-4 py-3 text-custom-sm font-bold text-ink transition-all duration-150 hover:border-ink hover:bg-sunny-yellow hover:shadow-brutal-sm xl:border-0 xl:rounded-none xl:px-3 xl:py-0 xl:font-semibold xl:hover:border-transparent xl:hover:bg-transparent xl:hover:text-green-600 xl:hover:shadow-none ${
                  stickyMenu ? "xl:py-4" : "xl:py-6"
                } ${variant === "mobile" ? "border-ink bg-white shadow-brutal-sm" : ""}`}
              >
                {menuItem.title}
              </Link>
            </li>
  ...
  return (
    <header
      className={`fixed left-0 top-0 z-9999 w-full border-b-2 border-ink bg-cream transition-all duration-300 ease-in-out ${
        stickyMenu && "shadow-brutal-sm"
      }`}
    >
      <div className="mx-auto max-w-[1170px] px-4 sm:px-7.5 xl:px-0">
        {/* Mobile: cart | logo | menu */}
        <div className="relative py-3 lg:hidden">
          ...
          {navigationOpen && (
            <div className="absolute left-0 right-0 top-full z-[10000] mx-0 mt-3 max-h-[min(70vh,420px)] overflow-y-auto rounded-brutal-lg border-2 border-ink bg-white p-4 shadow-brutal">
              <p className="mb-3 px-1 text-2xs font-bold uppercase tracking-widest text-dark-4">
                Menu
              </p>
              {navigationMenu(closeMobileNav, "mobile")}
            </div>
          )}
        </div>
        ...
        {/* Desktop header top */}
        <div
          className={`hidden flex-col gap-5 ease-out duration-200 lg:flex lg:flex-row lg:items-center xl:justify-between ${
          ...
          <div className="flex w-full flex-col items-center gap-5 sm:flex-row sm:justify-between sm:gap-10 xl:w-auto">
            ...
            <div className="relative hidden w-full max-w-[475px] lg:block" ref={searchRef}>
              <form onSubmit={handleSearchSubmit} className="relative w-full">
                <div className="header-search-bar flex w-full items-stretch overflow-hidden rounded-brutal border-2 border-ink bg-white shadow-brutal-sm">
                  <CustomSelect
                    options={options}
                    onSelectChange={setSelectedCategory}
                  />

                  <div className="relative flex min-w-0 flex-1 items-center border-l-2 border-ink/15">
                    <input
                      ...
                      autoComplete="off"
                      className="w-full min-w-0 border-0 bg-white py-2.5 pl-4 pr-11 text-custom-sm font-medium text-ink outline-none placeholder:text-dark-5"
                    />
                    ...
                    <button
                      ...
                      id="search-btn"
                      aria-label="Search"
                      className="absolute right-3 top-1/2 flex -translate-y-1/2 items-center justify-center text-ink transition-colors hover:text-green-600"
                    >
                      <svg
                ...
                {showSearchResults && searchResults.length > 0 && (
                  <div className="absolute left-0 right-0 top-[calc(100%+6px)] z-[100001] max-h-[400px] overflow-y-auto rounded-brutal border-2 border-ink bg-white shadow-brutal">
                    <div className="p-2">
                      {searchResults.map((product) => (
                        <div
                          key={product.id}
                          onClick={() => handleProductClick(product.id)}
                          className="flex cursor-pointer items-center gap-3 rounded-brutal p-2 transition-colors hover:bg-cream"
                        >
                          <div className="relative h-12 w-12 shrink-0">
                            <Image
                              ...
                              alt={product.title}
                              fill
                              className="rounded-brutal object-cover"
                              sizes="48px"
                            />
                          </div>
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="truncate text-xs text-dark-4">
      ...
      <div className="relative hidden border-t-2 border-ink/15 lg:block xl:static">
        <div className="mx-auto max-w-[1170px] px-4 sm:px-7.5 xl:px-0">
          <div className="flex items-center justify-between">
            {/* <!--=== Main Nav Start ===--> */}
            <div
              className={`absolute left-4 right-4 top-full z-[10000] h-0 w-auto max-w-[calc(100%-2rem)] items-center justify-between invisible lg:flex xl:static xl:left-auto xl:right-auto xl:z-auto xl:h-auto xl:max-w-none xl:visible ${
                navigationOpen &&
                "lg:!visible lg:!h-auto lg:max-h-[min(70vh,400px)] lg:overflow-y-auto lg:rounded-brutal-lg lg:border-2 lg:border-ink lg:bg-white lg:p-5 lg:shadow-brutal"
              }`}
            >
              {navigationMenu(() => setNavigationOpen(false), "desktop")}
            </div>
            {/* // <!--=== Main Nav End ===--> */}
            ...
            <div className="hidden xl:block">
              <ul className="flex items-center gap-5.5">
                ...
                <li className="py-4">
                  <Link
                    href="/wishlist"
                    className="flex items-center gap-1.5 rounded-brutal border-2 border-transparent px-3 py-1.5 font-semibold text-custom-sm text-ink transition-all duration-150 hover:border-ink hover:bg-white hover:shadow-brutal-sm"
                  >
                    <svg
                      className="fill-current"
                      width="16"
