...
const SingleItem = ({ item }: SingleItemProps) => {
  ...
  return (
    <div className="flex items-start gap-3 rounded-brutal-lg border-2 border-ink bg-white p-3.5 shadow-brutal-sm">
      <div className="flex h-[72px] w-[72px] shrink-0 items-center justify-center overflow-hidden rounded-brutal border-2 border-ink bg-cream">
        {productImage ? (
          <Image
            src={productImage}
            alt={item.title}
      ...
      <div className="min-w-0 flex-1 pt-0.5">
        ...
        <div className="mt-1.5 flex flex-wrap items-center gap-2">
          ...
          {item.quantity > 1 && (
            <span className="rounded-brutal border border-ink/20 bg-sunny-yellow px-1.5 py-0.5 text-xs font-bold text-ink">
              ×{item.quantity}
            </span>
          )}
        </div>
